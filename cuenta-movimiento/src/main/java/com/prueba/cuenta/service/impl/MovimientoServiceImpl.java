...
public class MovimientoServiceImpl implements MovimientoService {
    ...
    public List<MovimientoDTO> obtenerMovimientosPorFechas(List<String> numeroCuenta, LocalDate fechaInicio, LocalDate fechaFin) {
        for (String numero : numeroCuenta) {
            if (!cuentaRepository.findByNumeroCuenta(numero).isPresent()) {
                throw new CuentaNoEncontradaException("La cuenta número " + numero + " no fue encontrada.");
            }
        }
